...
class APIClient:
    ...
    def update_booking(self, booking_id, update_data):
        with allure.step('Update booking'):
            url = f"{self.base_url}{Endpoints.BOOKING_ENDPOINT.value}{booking_id}"
            response = requests.put(url, json=update_data, auth=HTTPBasicAuth(Users.USERNAME.value, Users.PASSWORD.value))
            response.raise_for_status()
        with allure.step('Checking status code'):
            assert response.status_code == 200, f"Expected status 200 but got {response.status_code}"
        return response.json()
    ...
    def partial_update_booking(self, booking_id, partial_update_data):
        with allure.step('Partial Update Booking'):
            url = f"{self.base_url}{Endpoints.BOOKING_ENDPOINT.value}/{booking_id}"
            response = self.session.patch(url, json=partial_update_data, auth=HTTPBasicAuth(Users.USERNAME.value, Users.PASSWORD.value))
            response.raise_for_status()
        with allure.step('Checking status code'):
            assert response.status_code == 200, f"Expected status 200 but got {response.status_code}"
        return response.json()
